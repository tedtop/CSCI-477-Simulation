...
simulator = RespawningHourglassSimulator(
    N=500,                  # Fewer particles to better see individual behavior
    Lx=20.0,                # Box width
    Ly=20.0,                # Box height
    dt=0.005,               # Small time step for stability
    gravity=5.0,            # Standard gravity
    particle_radius=0.4,    # Visible particle size
    k=30.0,                 # Medium-stiff spring constant
    gamma=5.0,              # Higher damping for less bouncing
    friction_coef=0.7,      # Medium-high friction for sand-like behavior
    restitution_coef=0.2,   # Low restitution (less bouncy)
    neck_width=1.0,         # Moderately narrow neck
